...
def resample_u( t_pwm, u ):
    (Nin,Nhaar) = u.shape

    u_pwm = np.zeros( ( Nin, t_pwm.size ) )

    idx = 0
    for k in range( t_pwm.size ):
        if( t_pwm[k] >= float(idx+1)/Nhaar + np.spacing(1) ):
            idx = idx + 1
        # print( "k: {0}, idx: {1}".format(k,idx) )
        # print( "t_pwm: {0}, t_haar: {1}".format(t_pwm[k],float(idx)/Nhaar) )
        u_pwm[:,k] = u[:,idx]
